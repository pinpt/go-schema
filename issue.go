...
// FindIssuesByState will find all Issues by the State value
func FindIssuesByState(ctx context.Context, db *sql.DB, value Issue_IssueState) ([]*Issue, error) {
	q := "SELECT `issue`.`id`,`issue`.`checksum`,`issue`.`user_id`,`issue`.`project_id`,`issue`.`title`,`issue`.`body`,`issue`.`state`,`issue`.`type`,`issue`.`resolution`,`issue`.`created_at`,`issue`.`updated_at`,`issue`.`closed_at`,`issue`.`closedby_user_id`,`issue`.`url`,`issue`.`customer_id`,`issue`.`ref_type`,`issue`.`ref_id`,`issue`.`metadata` FROM `issue` WHERE `state` = ? LIMIT 1"
...
// FindIssuesByStateTx will find all Issues by the State value using the provided transaction
func FindIssuesByStateTx(ctx context.Context, tx *sql.Tx, value Issue_IssueState) ([]*Issue, error) {
	q := "SELECT `issue`.`id`,`issue`.`checksum`,`issue`.`user_id`,`issue`.`project_id`,`issue`.`title`,`issue`.`body`,`issue`.`state`,`issue`.`type`,`issue`.`resolution`,`issue`.`created_at`,`issue`.`updated_at`,`issue`.`closed_at`,`issue`.`closedby_user_id`,`issue`.`url`,`issue`.`customer_id`,`issue`.`ref_type`,`issue`.`ref_id`,`issue`.`metadata` FROM `issue` WHERE `state` = ? LIMIT 1"
...
// FindIssuesByType will find all Issues by the Type value
func FindIssuesByType(ctx context.Context, db *sql.DB, value Issue_IssueType) ([]*Issue, error) {
	q := "SELECT `issue`.`id`,`issue`.`checksum`,`issue`.`user_id`,`issue`.`project_id`,`issue`.`title`,`issue`.`body`,`issue`.`state`,`issue`.`type`,`issue`.`resolution`,`issue`.`created_at`,`issue`.`updated_at`,`issue`.`closed_at`,`issue`.`closedby_user_id`,`issue`.`url`,`issue`.`customer_id`,`issue`.`ref_type`,`issue`.`ref_id`,`issue`.`metadata` FROM `issue` WHERE `type` = ? LIMIT 1"
	rows, err := db.QueryContext(ctx, q, orm.ToSQLString(value))
	if err == sql.ErrNoRows {
		return nil, nil
	}
...
// FindIssuesByTypeTx will find all Issues by the Type value using the provided transaction
func FindIssuesByTypeTx(ctx context.Context, tx *sql.Tx, value Issue_IssueType) ([]*Issue, error) {
	q := "SELECT `issue`.`id`,`issue`.`checksum`,`issue`.`user_id`,`issue`.`project_id`,`issue`.`title`,`issue`.`body`,`issue`.`state`,`issue`.`type`,`issue`.`resolution`,`issue`.`created_at`,`issue`.`updated_at`,`issue`.`closed_at`,`issue`.`closedby_user_id`,`issue`.`url`,`issue`.`customer_id`,`issue`.`ref_type`,`issue`.`ref_id`,`issue`.`metadata` FROM `issue` WHERE `type` = ? LIMIT 1"
	rows, err := tx.QueryContext(ctx, q, orm.ToSQLString(value))
	if err == sql.ErrNoRows {
		return nil, nil
...
func DBCreateIssueTable(ctx context.Context, db *sql.DB) error {
	q := "CREATE TABLE `issue` (`id`VARCHAR(64) NOT NULL PRIMARY KEY,`checksum`CHAR(64),`user_id` VARCHAR(64),`project_id` VARCHAR(64) NOT NULL,`title`TEXT NOT NULL,`body` LONGTEXT,`state`ENUM('open','closed') NOT NULL,`type` ENUM('bug','feature','enhancement','other') NOT NULL,`resolution` ENUM('none','resolved','unresolved') NOT NULL,`created_at` BIGINT(20) UNSIGNED NOT NULL,`updated_at` BIGINT(20) UNSIGNED,`closed_at` BIGINT(20) UNSIGNED,`closedby_user_id` VARCHAR(64),`url` VARCHAR(255) NOT NULL,`customer_id`VARCHAR(64) NOT NULL,`ref_type`VARCHAR(20) NOT NULL,`ref_id` VARCHAR(64) NOT NULL,`metadata`JSON,INDEX issue_state_index (`state`),INDEX issue_type_index (`type`),INDEX issue_customer_id_index (`customer_id`),INDEX issue_ref_type_index (`ref_type`),INDEX issue_ref_id_index (`ref_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"
