...
func TestCreateJiraIssueCommentTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraIssueCommentTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraIssueCommentDelete(t *testing.T) {
	r := &JiraIssueComment{
		ID:         "08cc98b2f33742d0",
		Checksum:   nil,
		CommentID:  "87fec9c8bd145192",
		IssueID:    "c195d5e1fb617ca1",
		ProjectID:  "831d4df1f7802715",
		UserID:     nil,
		UpdatedAt:  int64(64),
		CustomerID: "e4b57016653e640f",
		RefID:      "4f7db21148ef26d7",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraIssueComments(ctx, db)
	...
	found, err := FindJiraIssueCommentByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueComments(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCommentID("d503670b6b6e5b19")

	r.SetIssueID("7db78189b12a81b4")

	r.SetProjectID("76e935e890256467")

	r.SetUpdatedAt(int64(640))

	r.SetCustomerID("5f5c14c21f619821")

	r.SetRefID("271eb00ed808f1bb")
...
func TestCreateJiraIssueCommentDeleteTx(t *testing.T) {
	r := &JiraIssueComment{
		ID:         "08cc98b2f33742d0",
		Checksum:   nil,
		CommentID:  "87fec9c8bd145192",
	...
	found, err := FindJiraIssueCommentByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueCommentsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
