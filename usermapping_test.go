...
func TestCreateUserMappingTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateUserMappingTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateUserMappingDelete(t *testing.T) {
	r := &UserMapping{
		ID:         "7071f314c6ef57a1",
		Checksum:   nil,
		CustomerID: "ce6b22dd2ae84a63",
		UserID:     "77bd5f860c97a840",
		RefID:      "2506adf82ac24e24",
		RefType:    "0c89cbaa6a25a047",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllUserMappings(ctx, db)
	...
	found, err := FindUserMappingByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindUserMappings(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("02489690c77693ae")

	r.SetUserID("9bd3086a1a47307b")

	r.SetRefID("ff296711609d155b")

	r.SetRefType("eb4f35a8b443ef2d")
...
func TestCreateUserMappingDeleteTx(t *testing.T) {
	r := &UserMapping{
		ID:         "7071f314c6ef57a1",
		Checksum:   nil,
		CustomerID: "ce6b22dd2ae84a63",
	...
	found, err := FindUserMappingByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindUserMappingsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
