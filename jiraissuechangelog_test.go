...
func TestCreateJiraIssueChangeLogTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraIssueChangeLogTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraIssueChangeLogDelete(t *testing.T) {
	r := &JiraIssueChangeLog{
		ID:         "7981066355bceede",
		UserID:     nil,
		AssigneeID: nil,
		CreatedAt:  nil,
		Field:      "4ecad61303ce8b8c",
		FieldType:  nil,
		From:       nil,
		FromString: nil,
		To:         nil,
		ToString:   nil,
		RefID:      "6f68079189f5fe7d",
	...
	db := GetDatabase()
	DeleteAllJiraIssueChangeLogs(ctx, db)
	...
	found, err := FindJiraIssueChangeLogByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueChangeLogs(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
...
func TestCreateJiraIssueChangeLogDeleteTx(t *testing.T) {
	r := &JiraIssueChangeLog{
		ID:         "7981066355bceede",
		UserID:     nil,
		AssigneeID: nil,
	...
	found, err := FindJiraIssueChangeLogByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueChangeLogsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
