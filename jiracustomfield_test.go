...
func TestCreateJiraCustomFieldTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraCustomFieldTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraCustomFieldDelete(t *testing.T) {
	r := &JiraCustomField{
		ID:             "402506d88b757571",
		Checksum:       nil,
		FieldID:        "086bd6455a31a93e",
		Name:           "d6b3813b80da1675",
		SchemaType:     "089413055bb1e6b1",
		SchemaCustom:   "785193b04c969361",
		SchemaCustomID: nil,
		CustomerID:     "3656c0bf86e2953a",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraCustomFields(ctx, db)
	...
	found, err := FindJiraCustomFieldByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraCustomFields(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetFieldID("7884d2e70abb2a97")

	r.SetName("fb0a4eb4034fe460")

	r.SetSchemaType("ac810b52a67ca8d7")

	r.SetSchemaCustom("86962711a51e019e")

	r.SetCustomerID("5cac8ca429465378")
...
func TestCreateJiraCustomFieldDeleteTx(t *testing.T) {
	r := &JiraCustomField{
		ID:             "402506d88b757571",
		Checksum:       nil,
		FieldID:        "086bd6455a31a93e",
	...
	found, err := FindJiraCustomFieldByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraCustomFieldsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
