...
func TestCreateNewrelicApplicationTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateNewrelicApplicationTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateNewrelicApplicationDelete(t *testing.T) {
	r := &NewrelicApplication{
		ID:           "d32e31a7b710b7fe",
		Checksum:     nil,
		CustomerID:   "44f9fad9443a09e1",
		ExtID:        int64(64),
		Name:         "7029da7a083205cd",
		HealthStatus: nil,
		Reporting:    true,
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllNewrelicApplications(ctx, db)
	...
	found, err := FindNewrelicApplicationByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicApplications(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("c51939ae5cfa927d")

	r.SetExtID(int64(640))

	r.SetName("2e2a9e7b4bdea5ca")
...
func TestCreateNewrelicApplicationDeleteTx(t *testing.T) {
	r := &NewrelicApplication{
		ID:           "d32e31a7b710b7fe",
		Checksum:     nil,
		CustomerID:   "44f9fad9443a09e1",
	...
	found, err := FindNewrelicApplicationByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicApplicationsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
