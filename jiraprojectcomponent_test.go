...
func TestCreateJiraProjectComponentTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraProjectComponentTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraProjectComponentDelete(t *testing.T) {
	r := &JiraProjectComponent{
		ID:          "e3d4a892e15f7c16",
		Checksum:    nil,
		ComponentID: "156fb2c109bfcc56",
		ProjectID:   "a60e8f5e6fc6d26e",
		Name:        "bfb6bc329758ce6a",
		CustomerID:  "c0e12b9cc118c2e1",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraProjectComponents(ctx, db)
	...
	found, err := FindJiraProjectComponentByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectComponents(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetComponentID("84588e7578f15cb1")

	r.SetProjectID("db4ccc8a7400702e")

	r.SetName("ca7f41aa475e544c")

	r.SetCustomerID("7cbd4de5253b3444")
...
func TestCreateJiraProjectComponentDeleteTx(t *testing.T) {
	r := &JiraProjectComponent{
		ID:          "e3d4a892e15f7c16",
		Checksum:    nil,
		ComponentID: "156fb2c109bfcc56",
	...
	found, err := FindJiraProjectComponentByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectComponentsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
