...
func TestCreateUserCostCenterTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateUserCostCenterTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateUserCostCenterDelete(t *testing.T) {
	r := &UserCostCenter{
		ID:           "8d788f405046e5a6",
		Checksum:     nil,
		CustomerID:   "b78ffcce2b7e728f",
		UserID:       "1b49d01f11642bbb",
		CostCenterID: nil,
		UnitCost:     nil,
		UnitType:     nil,
		UnitCurrency: nil,
	...
	db := GetDatabase()
	DeleteAllUserCostCenters(ctx, db)
	...
	found, err := FindUserCostCenterByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindUserCostCenters(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("fe3c74ee4ab994fd")

	r.SetUserID("f58a7fdd764ebc61")
...
func TestCreateUserCostCenterDeleteTx(t *testing.T) {
	r := &UserCostCenter{
		ID:           "8d788f405046e5a6",
		Checksum:     nil,
		CustomerID:   "b78ffcce2b7e728f",
	...
	found, err := FindUserCostCenterByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindUserCostCentersTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
