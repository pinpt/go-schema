...
func TestCreateNewrelicAlertsIncidentTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateNewrelicAlertsIncidentTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateNewrelicAlertsIncidentDelete(t *testing.T) {
	r := &NewrelicAlertsIncident{
		ID:                 "aeb0f46aa0bbe31c",
		Checksum:           nil,
		CustomerID:         "adca86c5e6219dbd",
		ExtID:              int64(64),
		OpenedAt:           int64(64),
		ClosedAt:           nil,
		IncidentPreference: "a3af8252e3e754a5",
		ViolationExtIds:    "{}",
		ViolationIds:       "{}",
		PolicyExtID:        nil,
		PolicyID:           nil,
	...
	db := GetDatabase()
	DeleteAllNewrelicAlertsIncidents(ctx, db)
	...
	found, err := FindNewrelicAlertsIncidentByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicAlertsIncidents(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("5cdf580a42db69e8")

	r.SetExtID(int64(640))

	r.SetOpenedAt(int64(640))

	r.SetIncidentPreference("73c5516c1355fb5c")

	r.SetViolationExtIds("{}")
...
func TestCreateNewrelicAlertsIncidentDeleteTx(t *testing.T) {
	r := &NewrelicAlertsIncident{
		ID:                 "aeb0f46aa0bbe31c",
		Checksum:           nil,
		CustomerID:         "adca86c5e6219dbd",
	...
	found, err := FindNewrelicAlertsIncidentByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicAlertsIncidentsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
