...
func TestCreateUserTable(t *testing.T) {
	...
	err = DBCreateUserTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateUserDelete(t *testing.T) {
	r := &User{
		ID:              "8c935c62d229b992",
		Checksum:        nil,
		CustomerID:      "ee53df911064e1b1",
		Email:           nil,
		Name:            nil,
		Username:        "2031956ceffa28a7",
		Active:          true,
		Autoenrolled:    true,
		Visible:         true,
		AvatarURL:       "762f97703fc128ed",
		EmployeeID:      nil,
		Location:        nil,
		Region:          nil,
		Department:      nil,
	...
	found, err := FindUserByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	r.SetCustomerID("4ee6d4939461c20a")

	r.SetUsername("078f57c75998d0c6")

	r.SetActive(false)

	r.SetAutoenrolled(false)

	r.SetVisible(false)

	r.SetAvatarURL("a3361f7402b9a557")
...
func TestCreateUserDeleteTx(t *testing.T) {
	r := &User{
		ID:              "8c935c62d229b992",
		Checksum:        nil,
		CustomerID:      "ee53df911064e1b1",
	...
	found, err := FindUserByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindUsersTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
