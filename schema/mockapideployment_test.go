...
func TestCreateMockapiDeploymentTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateMockapiDeploymentTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateMockapiDeploymentDelete(t *testing.T) {
	r := &MockapiDeployment{
		ID:                    "7cf79ecd44610f9a",
		Checksum:              nil,
		CustomerID:            "0e83b6b711b5d003",
		ApplicationExtIDExtID: int64(64),
		ApplicationExtIDID:    "db90b12330f2cf9a",
		ExtID:                 int64(64),
		Name:                  "1a828c6e33e4eef5",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllMockapiDeployments(ctx, db)
	...
	found, err := FindMockapiDeploymentByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindMockapiDeployments(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("7088b1e32ad9c654")

	r.SetApplicationExtIDExtID(int64(640))

	r.SetApplicationExtIDID("c22dbe58e5ce937b")

	r.SetExtID(int64(640))

	r.SetName("5ae65c192561d7b5")
...
func TestCreateMockapiDeploymentDeleteTx(t *testing.T) {
	r := &MockapiDeployment{
		ID:                    "7cf79ecd44610f9a",
		Checksum:              nil,
		CustomerID:            "0e83b6b711b5d003",
	...
	found, err := FindMockapiDeploymentByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindMockapiDeploymentsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
