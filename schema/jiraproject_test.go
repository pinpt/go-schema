...
func TestCreateJiraProjectTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraProjectTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraProjectDelete(t *testing.T) {
	r := &JiraProject{
		ID:         "6c4576158bdbe0b6",
		Checksum:   nil,
		ProjectID:  "604d556d826b1a58",
		Key:        "58e65ba2b2d913bd",
		AvatarURL:  "0ff27a4df2852537",
		CategoryID: nil,
		CustomerID: "cf9677d470d86caf",
		RefID:      "d93be1c859e5a374",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraProjects(ctx, db)
	...
	found, err := FindJiraProjectByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjects(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetProjectID("9a8c4c92fde28534")

	r.SetKey("0912827bb479041a")

	r.SetAvatarURL("e40985a474c34d4a")

	r.SetCustomerID("73a6cac54d17f460")

	r.SetRefID("a1d3c48b781e9213")
...
func TestCreateJiraProjectDeleteTx(t *testing.T) {
	r := &JiraProject{
		ID:         "6c4576158bdbe0b6",
		Checksum:   nil,
		ProjectID:  "604d556d826b1a58",
	...
	found, err := FindJiraProjectByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
