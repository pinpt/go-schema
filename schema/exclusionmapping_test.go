...
func TestCreateExclusionMappingTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateExclusionMappingTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateExclusionMappingDelete(t *testing.T) {
	r := &ExclusionMapping{
		ID:         "fbf09d8b7c79d9b9",
		RefID:      "2cd3440b72f47fbc",
		RefType:    "9a5fe3c3a2967966",
		UserID:     nil,
		Date:       nil,
		CustomerID: "c304c511dd5511c6",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllExclusionMappings(ctx, db)
	...
	found, err := FindExclusionMappingByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindExclusionMappings(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
...
func TestCreateExclusionMappingDeleteTx(t *testing.T) {
	r := &ExclusionMapping{
		ID:         "fbf09d8b7c79d9b9",
		RefID:      "2cd3440b72f47fbc",
		RefType:    "9a5fe3c3a2967966",
	...
	found, err := FindExclusionMappingByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindExclusionMappingsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
