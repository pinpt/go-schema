...
func DBCreateNewrelicAlertsViolationTable(ctx context.Context, db *sql.DB) error {
	q := "CREATE TABLE `newrelic_alerts_violation` (`id`VARCHAR(64) NOT NULL PRIMARY KEY,`checksum`CHAR(64),`customer_id`VARCHAR(64) NOT NULL,`ext_id` BIGINT NOT NULL,`duration`BIGINT NOT NULL,`policy_name`TEXT NOT NULL,`condition_name`TEXT NOT NULL,`priority`TEXT NOT NULL,`opened_at` BIGINT NOT NULL,`entity_product`TEXT NOT NULL,`entity_type`TEXT NOT NULL,`entity_group_id` BIGINT NOT NULL,`entity_id` BIGINT NOT NULL,`entity_name`TEXT NOT NULL,`policy_ext_id` BIGINT NOT NULL,`policy_id` VARCHAR(64) NOT NULL,`condition_ext_id` BIGINT NOT NULL,`condition_id` VARCHAR(64) NOT NULL,INDEX newrelic_alerts_violation_customer_id_index (`customer_id`),INDEX newrelic_alerts_violation_policy_ext_id_index (`policy_ext_id`),INDEX newrelic_alerts_violation_policy_id_index (`policy_id`),INDEX newrelic_alerts_violation_condition_ext_id_index (`condition_ext_id`),INDEX newrelic_alerts_violation_condition_id_index (`condition_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"
