...
func TestCreateJiraIssueTypeTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraIssueTypeTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraIssueTypeDelete(t *testing.T) {
	r := &JiraIssueType{
		ID:          "d2bc90c9896639c8",
		Checksum:    nil,
		IssueTypeID: "257d66c512eb770c",
		Name:        "7e3eff61445a7e30",
		Description: nil,
		IconURL:     nil,
		URL:         nil,
		Subtask:     true,
		CustomerID:  "bcb47c002817216c",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraIssueTypes(ctx, db)
	...
	found, err := FindJiraIssueTypeByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueTypes(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetIssueTypeID("025b30ce9cc36110")

	r.SetName("6e1581a607feb8f1")

	r.SetSubtask(false)

	r.SetCustomerID("90c85dfbc5cb57b8")
...
func TestCreateJiraIssueTypeDeleteTx(t *testing.T) {
	r := &JiraIssueType{
		ID:          "d2bc90c9896639c8",
		Checksum:    nil,
		IssueTypeID: "257d66c512eb770c",
	...
	found, err := FindJiraIssueTypeByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueTypesTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
