...
func TestCreateNewrelicMetricTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateNewrelicMetricTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateNewrelicMetricDelete(t *testing.T) {
	r := &NewrelicMetric{
		ID:               "0c023a2d0e5aa311",
		Checksum:         nil,
		CustomerID:       "332b859b6ab9ef7f",
		ApplicationExtID: int64(64),
		ApplicationID:    "ba2a5543653f6ea0",
		Name:             "ed78ed46878af8f3",
		ValueName:        "779fe6ba96e021ce",
		Start:            int64(64),
		DurationSec:      int64(64),
		Value:            float64(6.4),
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllNewrelicMetrics(ctx, db)
	...
	found, err := FindNewrelicMetricByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicMetrics(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("fea97434f662603f")

	r.SetApplicationExtID(int64(640))

	r.SetApplicationID("cda88a6bc3242d2e")

	r.SetName("affe045eb36d9a2f")

	r.SetValueName("4e69dc21df67a810")

	r.SetStart(int64(640))

	r.SetDurationSec(int64(640))

	r.SetValue(float64(64.1))
...
func TestCreateNewrelicMetricDeleteTx(t *testing.T) {
	r := &NewrelicMetric{
		ID:               "0c023a2d0e5aa311",
		Checksum:         nil,
		CustomerID:       "332b859b6ab9ef7f",
	...
	found, err := FindNewrelicMetricByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicMetricsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
