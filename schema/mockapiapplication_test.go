...
func TestCreateMockapiApplicationTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateMockapiApplicationTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateMockapiApplicationDelete(t *testing.T) {
	r := &MockapiApplication{
		ID:         "02272640329e6eee",
		Checksum:   nil,
		CustomerID: "62402f0411437f08",
		ExtID:      int64(64),
		Name:       "c24079462e05226d",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllMockapiApplications(ctx, db)
	...
	found, err := FindMockapiApplicationByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindMockapiApplications(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("ae531769d79529d0")

	r.SetExtID(int64(640))

	r.SetName("59a13faf7d14e8e5")
...
func TestCreateMockapiApplicationDeleteTx(t *testing.T) {
	r := &MockapiApplication{
		ID:         "02272640329e6eee",
		Checksum:   nil,
		CustomerID: "62402f0411437f08",
	...
	found, err := FindMockapiApplicationByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindMockapiApplicationsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
