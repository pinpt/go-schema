...
func TestCreateNewrelicAlertsPolicyTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateNewrelicAlertsPolicyTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateNewrelicAlertsPolicyDelete(t *testing.T) {
	r := &NewrelicAlertsPolicy{
		ID:                 "87b51467466371ee",
		Checksum:           nil,
		CustomerID:         "fd005a88a550dc81",
		ExtID:              int64(64),
		IncidentPreference: "5f3e968c5c473bcd",
		Name:               "cb6f3d2cc770c5b2",
		CreatedAt:          int64(64),
		UpdatedAt:          int64(64),
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllNewrelicAlertsPolicies(ctx, db)
	...
	found, err := FindNewrelicAlertsPolicyByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicAlertsPolicies(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("f2f4994fc722b7f2")

	r.SetExtID(int64(640))

	r.SetIncidentPreference("da40b4cdb8d739f0")

	r.SetName("3e9993317e907f9c")
...
func TestCreateNewrelicAlertsPolicyDeleteTx(t *testing.T) {
	r := &NewrelicAlertsPolicy{
		ID:                 "87b51467466371ee",
		Checksum:           nil,
		CustomerID:         "fd005a88a550dc81",
	...
	found, err := FindNewrelicAlertsPolicyByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicAlertsPoliciesTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
