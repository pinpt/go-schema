...
func TestCreateCostCenterTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateCostCenterTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateCostCenterDelete(t *testing.T) {
	r := &CostCenter{
		ID:           "cb35e0a98c7aec48",
		Checksum:     nil,
		CustomerID:   "0bbbcc1b0512e5fd",
		Name:         "4c326eb95fc2e5b9",
		Description:  nil,
		Identifier:   nil,
	...
	found, err := FindCostCenterByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindCostCenters(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("5f76445d5c6fe34d")

	r.SetName("77df85c1579e25a6")

	r.SetUnitCost(float64(64.1))
...
func TestCreateCostCenterDeleteTx(t *testing.T) {
	r := &CostCenter{
		ID:           "cb35e0a98c7aec48",
		Checksum:     nil,
		CustomerID:   "0bbbcc1b0512e5fd",
	...
	found, err := FindCostCenterByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindCostCentersTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
