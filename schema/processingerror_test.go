...
func TestCreateProcessingErrorTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateProcessingErrorTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateProcessingErrorDelete(t *testing.T) {
	r := &ProcessingError{
		ID:         "08c28b56095b5911",
		Checksum:   nil,
		CreatedAt:  int64(64),
		Type:       "d5d6ccc2429c2a9d",
		Message:    "ac046f0e2d349f1e",
		Fatal:      true,
		CustomerID: "f8da24dfb036a701",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllProcessingErrors(ctx, db)
	...
	found, err := FindProcessingErrorByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindProcessingErrors(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetType("34db2db7e8a03d86")

	r.SetMessage("18636388d96db4ec")

	r.SetFatal(false)

	r.SetCustomerID("93c4f4d2e3ca33e5")
...
func TestCreateProcessingErrorDeleteTx(t *testing.T) {
	r := &ProcessingError{
		ID:         "08c28b56095b5911",
		Checksum:   nil,
		CreatedAt:  int64(64),
	...
	found, err := FindProcessingErrorByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindProcessingErrorsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
