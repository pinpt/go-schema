...
func TestCreateNewrelicDeploymentTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateNewrelicDeploymentTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateNewrelicDeploymentDelete(t *testing.T) {
	r := &NewrelicDeployment{
		ID:               "41df9dec64b5f2ff",
		Checksum:         nil,
		CustomerID:       "8920e19239d12b15",
		ApplicationExtID: int64(64),
		ApplicationID:    "3dc94b6c3d094d7d",
		ExtID:            int64(64),
		Revision:         "c9cb749e71096e3b",
		Changelog:        nil,
		Description:      nil,
		User:             "572c70ee1d13cda3",
		Timestamp:        int64(64),
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllNewrelicDeployments(ctx, db)
	...
	found, err := FindNewrelicDeploymentByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicDeployments(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("9234231a9187db04")

	r.SetApplicationExtID(int64(640))

	r.SetApplicationID("cabc939af68beb44")

	r.SetExtID(int64(640))

	r.SetRevision("7a6ec1e3eca2b710")

	r.SetUser("fc1bd8bdc2e61187")

	r.SetTimestamp(int64(640))
...
func TestCreateNewrelicDeploymentDeleteTx(t *testing.T) {
	r := &NewrelicDeployment{
		ID:               "41df9dec64b5f2ff",
		Checksum:         nil,
		CustomerID:       "8920e19239d12b15",
	...
	found, err := FindNewrelicDeploymentByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindNewrelicDeploymentsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
