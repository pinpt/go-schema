...
func TestCreateJiraIssueLabelTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraIssueLabelTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraIssueLabelDelete(t *testing.T) {
	r := &JiraIssueLabel{
		ID:         "8eef956447dc8460",
		Checksum:   nil,
		Name:       "69f101b374f8caa9",
		CustomerID: "6b3f212ad411f1e1",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraIssueLabels(ctx, db)
	...
	found, err := FindJiraIssueLabelByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueLabels(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetName("5727a4ee35173b35")

	r.SetCustomerID("4a31827acdc7e3e8")
...
func TestCreateJiraIssueLabelDeleteTx(t *testing.T) {
	r := &JiraIssueLabel{
		ID:         "8eef956447dc8460",
		Checksum:   nil,
		Name:       "69f101b374f8caa9",
	...
	found, err := FindJiraIssueLabelByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueLabelsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
