...
func TestCreateACLUserRoleTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateACLUserRoleTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateACLUserRoleDelete(t *testing.T) {
	r := &ACLUserRole{
		ID:          "6129a1742f669cd1",
		Checksum:    nil,
		UserID:      "5551f64297fc0384",
		RoleID:      "e4eb44e5d9640f61",
		CustomerID:  "541ae90979acfda1",
		AdminUserID: "499ff766cc70c0da",
		CreatedAt:   int64(64),
		UpdatedAt:   nil,
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllACLUserRoles(ctx, db)
	...
	found, err := FindACLUserRoleByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindACLUserRoles(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetUserID("5a35d3ab5c69ecda")

	r.SetRoleID("f8424dc546acde43")

	r.SetCustomerID("6b4b2c4b0252b323")

	r.SetAdminUserID("510a7fb3ab30f392")
...
func TestCreateACLUserRoleDeleteTx(t *testing.T) {
	r := &ACLUserRole{
		ID:          "6129a1742f669cd1",
		Checksum:    nil,
		UserID:      "5551f64297fc0384",
	...
	found, err := FindACLUserRoleByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindACLUserRolesTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
