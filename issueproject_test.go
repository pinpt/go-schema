...
func TestCreateIssueProjectTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateIssueProjectTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateIssueProjectDelete(t *testing.T) {
	r := &IssueProject{
		ID:         "c7d8c3e33f8ae702",
		Checksum:   nil,
		Name:       "065b433ce8c4d688",
		URL:        "7adebf41b2695e18",
		CreatedAt:  int64(64),
		CustomerID: "7c8e2d91b8b3631a",
		RefType:    "a796c3eee738a687",
		RefID:      "1fef92a5809fd261",
		Metadata:   nil,
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllIssueProjects(ctx, db)
	...
	found, err := FindIssueProjectByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindIssueProjects(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetName("85a9da3d805ab703")

	r.SetURL("4e1571ff3f696f41")

	r.SetCreatedAt(int64(640))

	r.SetCustomerID("803050d4ceac78a9")

	r.SetRefType("e86ebbfe7d83467d")

	r.SetRefID("07d769b04a916eff")
...
func TestCreateIssueProjectDeleteTx(t *testing.T) {
	r := &IssueProject{
		ID:         "c7d8c3e33f8ae702",
		Checksum:   nil,
		Name:       "065b433ce8c4d688",
	...
	found, err := FindIssueProjectByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindIssueProjectsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
