...
func TestCreateJiraIssueProgressTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraIssueProgressTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraIssueProgressDelete(t *testing.T) {
	r := &JiraIssueProgress{
		ID:         "9bcb38c78a6f2b59",
		Checksum:   nil,
		UserID:     nil,
		IssueID:    "6655b802aae2016f",
		StartDate:  nil,
		EndDate:    nil,
		Duration:   nil,
		CustomerID: "8359fe9ed0288cea",
		RefID:      "2eb779b9a2c3417c",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraIssueProgresses(ctx, db)
	...
	found, err := FindJiraIssueProgressByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueProgresses(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetIssueID("b3d9469ba86e2e9a")

	r.SetCustomerID("b5372adce2969d8f")

	r.SetRefID("1d8c83d80d09a18d")
...
func TestCreateJiraIssueProgressDeleteTx(t *testing.T) {
	r := &JiraIssueProgress{
		ID:         "9bcb38c78a6f2b59",
		Checksum:   nil,
		UserID:     nil,
	...
	found, err := FindJiraIssueProgressByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraIssueProgressesTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
