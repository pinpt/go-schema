...
func TestCreateRepoTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateRepoTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateRepoDelete(t *testing.T) {
	r := &Repo{
		ID:          "8b22b134fba389c9",
		Checksum:    nil,
		Name:        "62051f8db2f6e410",
		URL:         "a63508cab359d8c6",
		Active:      true,
		Fork:        true,
		ParentID:    nil,
		Description: nil,
		CreatedAt:   int64(64),
		UpdatedAt:   nil,
		CustomerID:  "be752f14bb075d64",
		RefType:     "acd1bac7807a9f99",
		RefID:       "f7d81ce466ee2b8a",
		Metadata:    nil,
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllRepos(ctx, db)
	...
	found, err := FindRepoByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	r.SetName("1f12083306443b73")

	r.SetURL("8c2b2dd6cf47af80")
	...
	r.SetCustomerID("19c67446e3775871")

	r.SetRefType("f5ee2961e22eb0f9")

	r.SetRefID("1163b9fe7331f6f7")
...
func TestCreateRepoDeleteTx(t *testing.T) {
	r := &Repo{
		ID:          "8b22b134fba389c9",
		Checksum:    nil,
		Name:        "62051f8db2f6e410",
	...
	found, err := FindRepoByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindReposTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
