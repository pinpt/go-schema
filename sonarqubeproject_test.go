...
func TestCreateSonarqubeProjectTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateSonarqubeProjectTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateSonarqubeProjectDelete(t *testing.T) {
	r := &SonarqubeProject{
		ID:         "7c24727f3f076237",
		Checksum:   nil,
		CustomerID: "eac5614d6191d321",
		ExtID:      "7419a87b0dcd3e01",
		Key:        "af18fa924b45a397",
		Name:       "581351c5eab955f0",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllSonarqubeProjects(ctx, db)
	...
	found, err := FindSonarqubeProjectByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindSonarqubeProjects(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("668776d74e49b873")

	r.SetExtID("0b810213efca886a")

	r.SetKey("037d37746cf7521b")

	r.SetName("4f5fd17760d453d3")
...
func TestCreateSonarqubeProjectDeleteTx(t *testing.T) {
	r := &SonarqubeProject{
		ID:         "7c24727f3f076237",
		Checksum:   nil,
		CustomerID: "eac5614d6191d321",
	...
	found, err := FindSonarqubeProjectByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindSonarqubeProjectsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
