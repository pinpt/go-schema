...
func TestCreateSignalTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateSignalTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateSignalDelete(t *testing.T) {
	r := &Signal{
		ID:         "74672476d72dcae7",
		Name:       "f3f52e81bbb80a58",
		Value:      float64(6.4),
		Timeunit:   int32(32),
		Date:       "2006-01-02",
		Metadata:   nil,
		CustomerID: "63d8bbc991bf4d0c",
		RefType:    nil,
		RefID:      nil,
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllSignals(ctx, db)
	...
	found, err := FindSignalByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindSignals(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
...
func TestCreateSignalDeleteTx(t *testing.T) {
	r := &Signal{
		ID:         "74672476d72dcae7",
		Name:       "f3f52e81bbb80a58",
		Value:      float64(6.4),
	...
	found, err := FindSignalByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindSignalsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
