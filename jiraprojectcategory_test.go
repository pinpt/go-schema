...
func TestCreateJiraProjectCategoryTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraProjectCategoryTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraProjectCategoryDelete(t *testing.T) {
	r := &JiraProjectCategory{
		ID:          "d6a15c3c8f76924b",
		Checksum:    nil,
		Name:        nil,
		Description: nil,
		CustomerID:  "6e5b05f2284cb67c",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraProjectCategories(ctx, db)
	...
	found, err := FindJiraProjectCategoryByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectCategories(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("498baa54fa1b76b5")
...
func TestCreateJiraProjectCategoryDeleteTx(t *testing.T) {
	r := &JiraProjectCategory{
		ID:          "d6a15c3c8f76924b",
		Checksum:    nil,
		Name:        nil,
	...
	found, err := FindJiraProjectCategoryByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectCategoriesTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
