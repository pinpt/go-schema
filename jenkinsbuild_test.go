...
func TestCreateJenkinsBuildTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJenkinsBuildTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJenkinsBuildDelete(t *testing.T) {
	r := &JenkinsBuild{
		ID:                 "696c4f6c0d942293",
		Checksum:           nil,
		CustomerID:         "25c00d5298c5a06c",
		JenkinsBuildNumber: int64(64),
		JobID:              "88ebd63e23cd0dba",
		Result:             JenkinsBuildResult_RESULT_UNKNOWN,
		CreatedAt:          int64(64),
		FinishedAt:         nil,
		Sha:                nil,
		Branch:             nil,
		URL:                "149fb117f0dd8ad4",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJenkinsBuilds(ctx, db)
	...
	found, err := FindJenkinsBuildByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJenkinsBuilds(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("8519398a25999ed1")

	r.SetJenkinsBuildNumber(int64(640))

	r.SetJobID("8b4c84f04e9b1733")

	r.SetResult(JenkinsBuildResult_RESULT_UNKNOWN)

	r.SetCreatedAt(int64(640))

	r.SetURL("6ca867029fce8831")
...
func TestCreateJenkinsBuildDeleteTx(t *testing.T) {
	r := &JenkinsBuild{
		ID:                 "696c4f6c0d942293",
		Checksum:           nil,
		CustomerID:         "25c00d5298c5a06c",
	...
	found, err := FindJenkinsBuildByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJenkinsBuildsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
