...
func TestCreateDataGroupTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateDataGroupTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateDataGroupDelete(t *testing.T) {
	r := &DataGroup{
		ID:              "ce76d5b4aff906b5",
		Checksum:        nil,
		CustomerID:      "3608413274b6d8da",
		Name:            "e2028f2b494b8cf1",
		Description:     nil,
		ParentID:        nil,
		IssueProjectIds: nil,
		UserIds:         nil,
	...
	found, err := FindDataGroupByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindDataGroups(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetCustomerID("ce6cf41b371562e6")

	r.SetName("290ccbf8a1b7630b")
...
func TestCreateDataGroupDeleteTx(t *testing.T) {
	r := &DataGroup{
		ID:              "ce76d5b4aff906b5",
		Checksum:        nil,
		CustomerID:      "3608413274b6d8da",
	...
	found, err := FindDataGroupByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindDataGroupsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
