...
func TestCreateJiraProjectVersionTable(t *testing.T) {
	tx, err := GetDatabase().Begin()
	if err != nil {
		t.Fatal(err)
	}
	err = DBCreateJiraProjectVersionTableTx(context.Background(), tx)
	if err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
...
func TestCreateJiraProjectVersionDelete(t *testing.T) {
	r := &JiraProjectVersion{
		ID:              "201d8f425a489dcf",
		Checksum:        nil,
		VersionID:       "cda4b61873a31a9c",
		ProjectID:       "e437488283ef8b8e",
		Name:            "0292d54cdac21372",
		Description:     nil,
		Archived:        true,
		Released:        true,
		ReleaseDate:     nil,
		Overdue:         true,
		UserReleaseDate: nil,
		CustomerID:      "609b2bb6a8a033c5",
	}
	ctx := context.Background()
	db := GetDatabase()
	DeleteAllJiraProjectVersions(ctx, db)
	...
	found, err := FindJiraProjectVersionByID(ctx, db, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectVersions(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
	...
	r.SetVersionID("0a57425b264365e4")

	r.SetProjectID("9ff06ade086042bf")

	r.SetName("f0b46e86b31755f1")
	...
	r.SetCustomerID("d98d98ea36fd600d")
...
func TestCreateJiraProjectVersionDeleteTx(t *testing.T) {
	r := &JiraProjectVersion{
		ID:              "201d8f425a489dcf",
		Checksum:        nil,
		VersionID:       "cda4b61873a31a9c",
	...
	found, err := FindJiraProjectVersionByIDTx(ctx, tx, r.ID)
	if err != nil {
		t.Fatal(err)
	}
	if found == nil {
	...
	results, err := FindJiraProjectVersionsTx(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	if results == nil {
